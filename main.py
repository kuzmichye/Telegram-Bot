...
load_dotenv()
bot = telebot.TeleBot(environ["TELEGRAM_TOKEN"], parse_mode=None)
config_dict = get_default_config()
config_dict['language'] = 'eng'
owm = OWM(environ["OWM_TOKEN"])
...
def send_welcome(message):
	...
	bot.send_message(message.chat.id,
	                 'My name is Kuzmich. I am the Telegram Bot and I can do various things. Now I am going to show '
	                 'you what I can do (you will see buttons on your keyboard).')
	bot.send_message(message.chat.id,
	                 'Weather - to find out the weather of a city you enter. Play a game - to guess a random number '
	                 'from 1 to 100. How is it going? - to ask how your life is going. The word of the day - to print '
	                 'a random English word in order to increase your vocabulary')
	bot.send_message(message.chat.id,
	                 'If you press a button and nothing works, then do not worry. The bot is in progress and there can '
	                 'be  some mistakes. Just  press the button you need once again)')
			...
			case 'Weather':
				def answer(city):
					try:
						...
						bot.send_message(message.chat.id,
						                 'The bot could not find your city:( Use /start to call the menu and to enter a city once again')

				city = bot.send_message(message.chat.id, 'Enter your city:')
				bot.register_next_step_handler(city, answer)
			case 'The word of the day':
...
def callback(call):
	if call.data == 'good':
		...
		bot.send_message(call.message.chat.id, 'Use /start to call the menu')
	bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='How is it going?',
	                      reply_markup=None)
